...
`,Q=class extends i{constructor(){super(),this.active=!1,this.external=!1,this.indicator=!1,this.iconOnly=!1,this.flush=!1,this.subnavState=`closed`,this.id=this.id||Math.random().toString(36).substring(2,6)}connectedCallback(){super.connectedCallback(),this.subnavState=this.active?`open`:`closed`}toggleSubnav(e){e.preventDefault(),e.stopPropagation(),this.subnavState=this.subnavState===`open`?`closed`:`open`}renderIconItem(t){let n=`item-${this.id}`;return e`
      <a
    ...
    `}renderItem(t,n=!1){return e`
      <a
        class="${x({"nav-item":!0,"nav-item--prefixed":n,"nav-item--flush":this.flush})}"
        href="${this.href}"
        aria-current="${this.active?`page`:!1}"
      >
        ${n?this.renderPrefix():r}
        <slot id="${this.id}-label"></slot>
        ${this.renderSuffix(t)}
      </a>
    `}render(){let t=!!this.querySelector(`[slot="subnav"]`),n=!!this.icon||!!this.querySelector(`[slot="prefix"]`)||!!this.querySelector(`[slot="icon"]`);return e`
      <li>
        ${this.iconOnly?this.renderIconItem(t):this.renderItem(t,n)}
        ${t?e`
              <div
                class="subnav"
                id="${this.id}-subnav"
